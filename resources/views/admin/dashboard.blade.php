...
@section('content')
    ...
    @else
        ...
        @if(Auth::user()->type == 'owner')
            @if(Auth::user()->plan != 1)
                @if(Auth::user()->plan != '' && (Auth::user()->is_plan_purchased == 0 || Auth::user()->plan_expire_date < date('Y-m-d')) )
            ...
            <div class="col-xl-4 col-md-6">
                <div class="card card-stats">
                    <div class="card-body">
                        <div class="row">
                            <div class="col">
                                <h6 class="text-muted mb-1">{{__('Total Subtasks')}}</h6>
            ...
            <div class="col-xl-12 col-md-12">
                <div class="card card-fluid">
                    <div class="card-header">
                        <h6 class="mb-0">{{__('Subtasks Overview')}}</h6>
                        <small class="text-muted">{{__('Total Completed subtask in last 7 days')}}</small>
                    </div>
                    <div class="card-body">
        ...
        <div class="row">
            <div class="col-sm-12 col-sm-12 col-md-4 col-lg-4">
                <div class="card card-fluid">
                    <div class="card-header">
                        <h6 class="mb-0">{{__('Tasks Status')}}</h6>
                    </div>
                    <div class="card-body">
            ...
            <div class="col-xl-8 col-md-8">
                <div class="card card-fluid">
                    <div class="card-header border-0">
                        <h6 class="mb-0">{{__('Top Due Subtasks')}}</h6>
                    </div>
                    <div class="card-wrapper table_scroll p-3">
                        <div class="table-responsive" style="position: relative;">
                            <table class="table align-items-center">
                                <thead>
                                <tr>
                                    <th scope="col" class="sort" data-sort="name">{{__('Subtasks')}}</th>
                                    <th scope="col" class="sort" data-sort="budget">{{__('Tasks')}}</th>
                                    <th scope="col" class="sort" data-sort="status">{{__('Priority')}}</th>
                                        ...
                                        <th scope="row">
                                            <div class="media align-items-center">
                                                <div class="media-body ml-4">
                                                    <a href="{{ route('projects.tasks.index',$due_task->project->id) }}" class="name mb-0 h6 text-sm">{{ $due_task->title }}</a>
                                                </div>
                                            </div>
                                        </th>
    ...
    @else
        <script>
            ...
            var t = {
                ...
                series: [{name: "{{__('Tasks')}} ", data: {!! json_encode(array_values($home_data['task_overview'])) !!}}],
            ...
            var t1 = {
                ...
                series: [{name: "{{__('Timesheet hours')}} ", data: {!! json_encode(array_values($home_data['timesheet_logged'])) !!}}],
