            ...
            <table class="table align-items-center">
                ...
                @if(count($tasks) > 0)
                    @foreach($tasks as $task)
                        <tr>
                            <td>
                                <span class="h6 text-sm font-weight-bold mb-0"><a href="{{ route('projects.tasks.index',$task->project->id) }}">{{ $task->title }}</a></span>
                            ...
                            <td class="{{ (strtotime($task->end_date) < time()) ? 'text-danger' : '' }}">{{ \App\Models\Utility::getDateFormated($task->end_date) }}</td>
                            <td>
